...
PARENT="$(cd -- "$ROOT/.." && pwd)"
ARTIFACT_ROOT="${OPENCLAW_TELEGRAM_OVERLAY_ARTIFACT_DIR:-$ROOT/deployment/.build/telegram-overlay}"
TELEGRAM_OVERLAY_DIR="$ARTIFACT_ROOT/telegram"
TELEGRAM_PLUGIN_ROOT="${OPENCLAW_TELEGRAM_REPO:-$PARENT/openclaw-telegram-enhanced}"

mkdir -p "$TELEGRAM_OVERLAY_DIR"
rm -rf "$TELEGRAM_OVERLAY_DIR"/*
...
echo
echo "Preparing Telegram overlay artifact inputs from pinned source repo..."
echo "  telegram source : $TELEGRAM_PLUGIN_ROOT"
echo "  artifact output : $ARTIFACT_ROOT"
...
if disallowed:
    print(f"Managed Telegram overlay packlist for {plugin_root} includes non-runtime files:", file=sys.stderr)
...
echo "Prepared Telegram overlay artifact inputs:"
find "$ARTIFACT_ROOT" -type f -printf '  overlay %P\n' | sort
